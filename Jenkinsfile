pipeline {
    ...
    stages {
        stage('Build') {
            steps {
                script {
                    dockerImage = docker.build("ayeshaaltaf/ayeshaaltaf-portfolio:${env.BUILD_ID}")
                }
            }
        }
        stage('Push') {
                            ...
                            sshPublisherDesc(
                                ...
                                transfers: [sshTransfer(
                                    execCommand: """
                                        ...
                                        docker run -d --name ayeshaaltaf-portfolio-container -p 400:80 ayeshaaltaf/personal-portfolio:${env.BUILD_ID}
                                    """
                                )]
                            )
                        ]
